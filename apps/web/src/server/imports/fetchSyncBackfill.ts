...
export async function fetchSyncBackfill(input: {
  boardName: string;
  articleId: string;
  startFloor?: number;
  ...
  const response = await fetch(
    `${baseUrl.replace(/\/$/, "")}/api/sync/backfill?${params.toString()}`,
    {
      method: "GET",
      headers: {
        "X-Sync-Token": token,
  ...
  if (!response.ok) {
    throw new Error(`Sync backfill request failed: ${response.status}`);
  }
