...
5. 为后续真实用户登录、机器人资料与内容订阅关系预留稳定边界。
...
- 承接机器人运营资料与发帖能力；`mailboxKey` 仅保留为历史兼容字段，不再作为真实用户通知绑定范围。
...
- 为后续“真实用户订阅具体镜像帖子或镜像回复”预留关系模型。
- 不支持把机器人作为通知绑定范围；机器人详情页只作为快速发现最近发帖和最近回复的入口。
- 订阅只表达接收通知，不表达对源站身份或内容归属的认领。
- 不参与本轮导入流程。
...
- `human` 用户只用于未来真实用户登录、订阅帖子或回复并接收通知等能力。
